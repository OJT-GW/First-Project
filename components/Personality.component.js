import { showEditPersonalityForm } from "../services/Overlay.service.js";
import {
  sharePersonality,
  deletePersonality,
  getPersonalityCard,
  getPersonalityIndex,
} from "../services/Personality.service.js";
...
export function createPersonalityCard(personality) {
  ...
  editButton.addEventListener("click", () => {
    const personalityToEditIndex = Array.from(
      personalityCard.parentNode.children
    ).indexOf(personalityCard);
    if (personalityToEditIndex <= 0) {
      return;
    }
  ...
  input.addEventListener("change", () => {
    const personalityCards =
      document.getElementsByClassName("card-personality");
    if (input.checked) {
      // Darken all cards
